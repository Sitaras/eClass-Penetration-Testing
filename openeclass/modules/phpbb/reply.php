...
if (isset($submit) && $submit) {
	...
	$stmt1 = $conn->prepare("INSERT INTO posts (topic_id, forum_id, poster_id, post_time, poster_ip, nom, prenom) VALUES (?, ?, ?, ?, ?, ?, ?)");
	...
	if ($this_post) {
		$stmt2 = $conn->prepare("INSERT INTO posts_text (post_id, post_text) VALUES ( ?, ? )");
		$stmt2->bind_param("is", $this_post, $purifier->purify($message));
		$stmt2->execute();
		$stmt2->close();
		}
	$stmt3 = $conn->prepare("UPDATE topics SET topic_replies = topic_replies+1, topic_last_post_id = ?, topic_time = ? WHERE topic_id = ?");
	$stmt3->bind_param("iss", $this_post, $time,$topic);
	$stmt3->execute();
	$stmt3->close();

	$stmt4 = $conn->prepare("UPDATE forums SET forum_posts = forum_posts+1, forum_last_post_id = ? WHERE forum_id = ?");
	$stmt4->bind_param("ii", $this_post, $forum);
	$stmt4->execute();
	$stmt4->close();
		if (!$result) {
