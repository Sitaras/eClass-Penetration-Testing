...
if (!isset( $_POST['authors']) || !isset( $_POST['description']))
	...
	if (!$error) {
      ...
      // $dropbox_filename = safe_filename($format);
			...
			if ($author_clean == '')
			{
				$author_clean = getUserNameFromId($uid);
			}
			...
			if (!$error)
			{
				...
				new Dropbox_SentWork($uid, $dropbox_title, $description_clean, $author_clean, $dropbox_filename, $dropbox_filesize, $newWorkRecipients);
			}
		}
		chdir ($cwd);
	} //end if(!$error)
