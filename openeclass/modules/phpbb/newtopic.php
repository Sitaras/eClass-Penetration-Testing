...
if (isset($submit) && $submit) {
    ...
    if ($err) {
        ...
        if ($post_id) {
            ...
            $stmt->bind_param("is", $post_id, $purifier->purify($message));
            $stmt->execute();
            if ($stmt->errno) {
                echo $stmt->errno;
                die;
    ...
    $stmt = $conn->prepare("UPDATE forums SET forum_posts = forum_posts+1, forum_topics = forum_topics+1, forum_last_post_id = ? WHERE forum_id = ?");
