...
if($submit) {
	...
	$stmt = $conn->prepare("SELECT username FROM `$mysqlMainDb`.user WHERE username=?");
	$stmt->bind_param("s",$purifier->purify($uname));
	$stmt->execute();
	...
	if (!$all_set) {
		...
		$purifier = new HTMLPurifier(HTMLPurifier_Config::createDefault());
        $stmt->bind_param("sssssiisiis", $purifier->purify( mysql_real_escape_string($nom_form)),$purifier->purify( mysql_real_escape_string($prenom_form)),$purifier->purify( mysql_real_escape_string($uname))
