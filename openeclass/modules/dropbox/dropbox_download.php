...
$sql = "SELECT p.recipientId, f.uploaderId FROM `" . $dropbox_cnf["fileTbl"] . "` f
...
{
  if ($res['uploaderId'] != $uid && $res['recipientId'] != $uid){
    die("Forbidden!");
  }
}else{
  die("Cannot fetch recipientId");
}
