 ...
 * @version $Id: index.php,v 1.103 2009-09-17 11:01:00 jexi Exp $
 *
...
elseif (isset($_REQUEST['install1']) || isset($_REQUEST['back1']))
{
	...
	if (!is_dir("../config")) {
		...
		$myfile = fopen("../config/index.php", "a") or die("Unable to open file!"); fclose($myfile);
	}
	// courses directory
	...
	if(!$mkd){
		...
		$myfile = fopen("../courses/index.php", "a") or die("Unable to open file!"); fclose($myfile);
	}
	...
	if (!is_dir("../video")) {
		...
		$myfile = fopen("../video/index.php", "a") or die("Unable to open file!"); fclose($myfile);
	}
	...
	<td><form name='langform' action='". htmlspecialchars($_SERVER[PHP_SELF]) ."' method=\"post\">".selection($langLanguages, 'lang', $lang, 'onChange="document.langform.submit();"')."</form></td>
	</tr>
	<tr>
