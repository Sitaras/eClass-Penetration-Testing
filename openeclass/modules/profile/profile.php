	...
	if (!empty($_POST['token'])) {
		if (($_SESSION['token'] === $_POST['token'])) {
			...
			// everything is ok
				...
				$purifier = new HTMLPurifier(HTMLPurifier_Config::createDefault());
				$stmt->bind_param("sssssssi",$purifier->purify($nom_form),$purifier->purify($prenom_form),$purifier->purify($username_form),$purifier->purify($email_form),$purifier->purify($am_form),$persoStatus,$langcode,$_SESSION["uid"]);
				$stmt->execute();
...
if ((!isset($changePass)) || isset($_POST['submit'])) {
	...
	<td><input class='FormData_InputText' type=\"text\" size=\"40\" name=\"am_form\" value=\"$am_form\">
	</td>
    </tr>";
	$tool_content .= "<td><input  type=\"hidden\" name=\"token\" value='".$_SESSION['token']."'></td>";
	##[BEGIN personalisation modification]############
	if (isset($_SESSION['perso_is_active'])) {
