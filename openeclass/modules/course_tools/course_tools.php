...
if ($is_adminOfCourse && @$action == 1) {//upload html file
	...
	<input type ='hidden' name='token' value = '{$courseToken}'>
	<br></form>";
	draw($tool_content, 2, 'course_tools');
	exit();
}
	...
	if ($extToolsCount>0)  {
		...
		for ($i=0; $i <$extToolsCount; $i++) {
			...
			$tool_content .= "<td align='center'>
    			...
    			<a href=\"".htmlspecialchars($_SERVER['PHP_SELF']) . "?delete=" . $externalLinks[$i]['id']."&token=".$courseToken."\" onClick=\"return confirmation('".addslashes($externalLinks[$i]['text'])."');\">
